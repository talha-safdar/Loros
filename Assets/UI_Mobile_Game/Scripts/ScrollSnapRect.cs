...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    public Transform pageSelectionRect;
    ...
    // container with bottom buttons menu
    private List<GameObject> _pageSelectionObjects;
    ...
    void Start() {
        ...
        GlobalManager.instance.PageCount = _pageCount;
    ...
    {
        _showPageSelection = pageSelectionRect != null;
        if (_showPageSelection)
        {
            if (pageSelectionRect.childCount != _pageCount)
            {
            ...
            else
            {
                ...
                _pageSelectionObjects = new List<GameObject>();

                // cache all Image components into list
                for (int i = 0; i < pageSelectionRect.childCount; i++)
                {
                    GameObject Gobject = pageSelectionRect.GetChild(i).gameObject;
                    if (Gobject == null)
                    {
                        Debug.LogWarning("Page selection icon at position " + i + " is missing Image component");
                    }
                    _pageSelectionObjects.Add(Gobject);
                }
            }
        }
    }
    ...
    private void NextScreen() {
        ...
        if (_currentPage > 0 && _currentPage < GlobalManager.instance.PageCount - 1)
        {
            if (reset)
                reset = false;
            GlobalAudio.instance.PlayWhooshPages();
        }
        else if (_currentPage == GlobalManager.instance.PageCount-1)
        {
            if (!reset)
            {
                GlobalAudio.instance.PlayWhooshPages();
